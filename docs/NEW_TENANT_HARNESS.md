...
### J. 결재자 기본값을 서식마다 하드코딩해두면 로그인한 사람과 무관한 이름이 저장된다 (여러 층에 중복돼 있어 한 곳만 고치면 되살아난다)

- **증상**: QC-001 검사자 칸에 항상 "최민재"가 미리 채워져 있음. 다른 서식들도
  검토자·승인자 칸에 항상 같은 이름(이다은/권화선/최민재)이 기본값으로 떠
  있음 — 실제 로그인한 사람과 무관해서 확인 없이 그대로 저장될 위험이 있었다
  (HACCP 추적성 결함).
- **원인**: 하드코딩된 기본 서명자 이름이 최소 3개 층에 **독립적으로 중복**돼
  있었다 — (1) 각 엔진의 `emptyState()`가 `approvals` 객체에 이름을 직접 심음,
  (2) `dkj-util.js`의 `DEFAULT_SIGNERS`/`autoFillUser()`가 필드명별로 고정
  이름을 따로 채움, (3) `dkj-approval.js`의 `transformInputToStaffSelect()`와는
  별개로 `stageHtml()`(결재 단계 미리보기 렌더러)이 자기만의 하드코딩 폴백을
  또 갖고 있었음. **셋 중 하나만 고치면 나머지 층이 다시 그 이름을 채워 넣어서
  "고쳤는데 그대로"인 것처럼 보인다** — 실제로 이번 세션에서 처음엔 "공용
  함수 하나만 고치면 된다"고 판단했다가, 검색해보니 같은 이름이 다른 두 층에도
  박혀 있는 걸 뒤늦게 발견했다.
- **고침**: 세 층을 전부 동시에 고쳤다 — 모든 엔진의 `emptyState()`에서
  writer/reviewer/approver를 빈 문자열로 시작, `autoFillUser()`는 작성자류
  (`writer`/`inspector`/`monitorName`/`author`/`handler`)만 로그인한 사람
  이름으로 채우고 검토자·승인자에는 절대 기본값을 넣지 않도록 재작성,
  `stageHtml()`의 별도 폴백도 제거(값이 없으면 `—`로 표시). 덧붙여
  `capa-management.js`/`emergency-drills.js`/`haccp-team-settings.js`/
  `management-culture.js`처럼 애초에 `autoFillUser()`를 호출조차 안 하던
  화면 4곳은, 하드코딩을 걷어내기만 하면 "작성자 칸이 영원히 안 채워짐"으로
  퇴행하므로 `autoFillUser()` 호출을 새로 추가해야 했다.
- **교훈**: 하드코딩된 기본값(특히 사람 이름·조직명 같은 사업장 고유값)을
  발견하면, 그 문자열로 저장소 전체를 검색해 다른 파일에도 같은 값이 박혀
  있는지 전수 확인할 것 — 한 군데 고치고 "됐다"고 판단하면 다른 층이 되살린다.
  결재(작성→검토→승인)에서 **검토자·승인자에게는 절대 기본값을 넣지 않고
  직접 선택하게 하는 것**이 원칙이다 — 작성자만 로그인 사용자로 자동 채운다.

### K. 임시저장 디바운스 타이머가 실제 저장 뒤에도 살아있으면 "저장됨" 문구를 다시 덮어쓴다

- **증상**: 마지막 입력칸을 채우자마자 저장 버튼을 누르면, 화면에 "저장됨"이
  잠깐 떴다가 수백 ms 뒤 "임시저장 hh:mm:ss"로 다시 바뀜 — 실제로는 저장이
  됐는데 화면 문구만 잘못 덮어써서 작업자가 "저장이 안 됐나?" 하고 다시 누르게
  만든다.
- **원인**: 입력칸이 바뀔 때마다 400ms 디바운스로 로컬 임시저장(draft)을 거는
  `scheduleDraft()`/`draftTimer`가, 실제 저장(`save()`)이 성공한 뒤에도 안
  지워진 채 살아있어서, 몇백 ms 뒤 예약돼 있던 임시저장 콜백이 뒤늦게 실행되며
  상태 문구를 되돌렸다. 경쟁 상태(race condition)라 항상 재현되는 건 아니고
  "마지막 칸 입력 직후 바로 저장"할 때만 걸린다 — 그래서 오래 발견되지 않았다.
- **고침**: 5개 엔진 + `DKJ-H-01-01`/`-02`/`DKJ-QC-001`/`FR-014`/`FR-015`
  전용 스크립트까지 총 12개 파일의 `save()`/`saveRecord()`가 검증을 통과한
  직후 `clearTimeout(draftTimer)`를 호출하도록 동일하게 추가.
- **교훈**: 디바운스·지연 타이머를 쓰는 자동저장 로직은, "즉시 실행되는 확정
  동작"(진짜 저장)이 성공하는 순간 반드시 대기 중인 타이머를 취소해야 한다 —
  안 그러면 나중에 실행되는 지연 콜백이 최신 상태를 덮어쓸 수 있다. 새 엔진을
  추가하거나 복사해서 만들 때 이 `clearTimeout` 한 줄을 빠뜨리기 쉬우니 체크
  리스트에 넣을 것.
...
| 시스템 관리자 사번 `4343` | `js/dkj-auth.js`, `js/dkj-approval.js`, `js/dkj-backup-reminder.js`, `js/dkj-chatbot.js`, `js/dkj-i18n.js`, `js/records-archive.js`, `system-settings.html`, `js/system-settings.js` 등 8개 파일 | **자동화됨** — `python scripts/new-tenant-rename.py --admin-emp-id <새사번> ... --apply` |
...
| 사업장 고유 데이터 (13개 카탈로그) | `data/*.json` 전체 (`console-forms`, `doc-catalog`, `menu-catalog`, `mdr-catalog`, `products`, `production-master`, `process-line`, `drive-document-manifest`, `asset-sources` 등) | 문서 목록·제품 마스터 등 전부 새 사업장 내용으로 교체 필요. **직원 명단은 데이터 파일이 아닙니다** — `system-settings.html`(시스템 관리자 사번)에서 등록하는 계정 디렉터리가 정본이고, 배포 파일에 실명을 적지 않는 것이 방침입니다(`data/staff-roles.json` 은 2026-09-09 삭제). 그중 서식 하나를 새로 추가하는 등록 자체는 **자동화됨** — `python scripts/new-record-catalog-add.py --apply` (PART 5 참고). 고친 뒤 `python scripts/build-catalog-bundles.py` 필수 |
...
### 5-0. 오프라인 원본이 HWP(한글) 파일이면 — Read 툴로는 못 연다

새 사업장(특히 농협·관공서 계열)은 오프라인 원본이 `.hwp`(한글과컴퓨터 워드
프로세서)로 오는 경우가 흔하다. **`Read` 툴로 `.hwp`를 그대로 열면 대부분
알아볼 수 없는 바이너리 쓰레기만 나온다** — 가끔 우연히 읽히는 짧은 글자
조각 몇 개를 빼면 내용 파악이 안 된다.

- **원인**: HWP 5.0(한글 2002 이후 기본 포맷)은 OLE 컴파운드 파일이고, 본문
  텍스트가 들어있는 `BodyText/Section0`, `Section1`… 스트림은 zlib(raw
  deflate)로 압축된 UTF-16LE 레코드 스트림이다. 일반 텍스트 추출로는 못 읽는다.
  (`.hwpx`는 최신 XML/zip 기반 포맷이라 이 문제가 없다 — `file <파일>`로 먼저
  버전부터 확인한다.)
- **뚫는 법**: `pip install olefile` 로 설치한 뒤, 표준 `zlib`과 조합해 직접
  파싱한다.
  1. `FileHeader` 스트림을 열어 오프셋 36~40 바이트(리틀엔디안 4바이트 정수)의
     최하위 비트로 압축 여부를 확인한다.
  2. `BodyText/Section0`, `Section1`… 스트림을 순서대로 열어
     `zlib.decompress(data, -15)`(raw deflate, zlib 헤더 없음)로 압축을 푼다.
  3. 풀린 바이트를 레코드 단위로 순회한다 — 4바이트 레코드 헤더(하위 10비트
     = 태그ID, 다음 10비트 = 레벨, 상위 12비트 = 크기, 크기 필드가 `0xFFF`면
     바로 다음 4바이트가 실제 크기)로 하나씩 잘라내고, 태그ID `67`
     (`HWPTAG_PARA_TEXT`, `HWPTAG_BEGIN(0x10) + 51`)인 레코드만 UTF-16LE로
     디코드한다(코드값 1~31은 인라인 컨트롤 문자라 건너뛴다).
  - 실제로 이 방법으로 `DKJ-S-02-07`(방충방서 점검일지) 개정본의 쥐트랩
    섹션을 정확히 추출해 온라인 서식과 비교·반영했다(2026-09-04~05, PR #64).
- **교훈**: `Read` 툴이 `.hwp`에서 바이너리 쓰레기만 뱉으면 Hancom 소프트웨어가
  없어도 이 방식으로 곧장 전환할 것 — 사람이 한글 프로그램을 열어 옮겨 적어줄
  때까지 기다릴 필요가 없다.
...
**작성자/검토자/승인자 칸에 특정 사람 이름을 기본값으로 미리 채우지 말 것**
(PART 2-J). 새 서식을 기존 서식 복사로 시작하면 예전에 있던 하드코딩 패턴을
그대로 베낄 위험이 있다 — 작성자만 로그인한 사람 이름으로 자동 채우고,
검토·승인자는 항상 빈 값으로 시작해 담당자가 직접 고르게 한다.
