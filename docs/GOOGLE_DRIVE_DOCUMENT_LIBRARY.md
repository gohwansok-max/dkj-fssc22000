...
CLAUDE.md 의 방침대로 배포되는 정적 파일(이 매니페스트 포함)에는 직원 실명을 넣지
않습니다. 원본 파일명 자체에 실명이 들어 있으면(예: "…_자격인정자료_홍길동_IA-…")
`scripts/build_drive_document_manifest.py` 의 `REDACT_NAMES` 목록에 있는 이름을
자동으로 지우고 매니페스트를 만듭니다(2026-09-09 추가) — Drive 링크(id 기반이라
파일명과 무관)는 그대로 동작하고, 웹에 노출되는 파일명·경로만 이름이 빠집니다.
스크립트를 돌리면 무엇을 지웠는지 콘솔에 출력합니다.

**이건 임시방편입니다.** 근본 해결은 Drive 원본 파일명을 실명 없이 바꾸는
것입니다(예: "…_자격인정자료_IA-…") — 문서 담당자가 Drive 에서 직접 리네임하세요.
새로 실명이 들어간 파일을 발견하면 `REDACT_NAMES` 에 이름을 추가하고, 이미 Drive
쪽 파일명을 바로잡았다면 목록에서 지워도 됩니다.
