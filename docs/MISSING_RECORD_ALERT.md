...
`DKJ-S-02-09`(세척소독제 관리)는 원래 이 절에서 "요일이 불명확해 제외"로
적혀 있었지만, 2026-09-09 이후 `daily` 그룹으로 옮겨져(`console-forms.json`)
지금은 `daily` 그룹의 `dayRow` 로직으로 매일 정상 감시된다 — 아래 내용은
`DKJ-S-02-13` 한 종에만 적용된다.

`weekly` 그룹의 `dayColumn` 서식 `DKJ-S-02-13`(저수조 관리)는 **의도적으로
이번 알림 대상에서 뺐다.** 이 서식은 실제로는 "매주 금요일 점검 · 월 단위
시트 5주치를 모아 한 번에 결재"하는 주기다(`check.dayMode:'week'`,
`weekStartDay:5` — `js/dkj-console.js`의 `evaluate()`가 이 주기를 안다).
그런데 이 서식처럼 **일주일·한 달 단위로 쓰고 한 번에 결재하는 대장/매트릭스
서식**은, 매일 마감을 재촉하는 이 알림의 취지(퇴근 전 마지막 확인)와 맞지
않는다 — 결재 전에 한동안 "진행 중"인 게 정상이라 매일 미작성 취급하면
안 된다(2026-09-18, 사용자 요청). 그래서 `weekly` 그룹의 `dayColumn`/`dayRow`
서식은 실제 운영 주기를 알아도 이 알림 대상에 넣지 않기로 했다 — 화면
(`js/dkj-console.js`)에서 "이번 주/달 진행 중" 배지로 충분히 안내된다.
