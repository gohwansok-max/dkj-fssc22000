...
### 대장 정본을 A4 1쪽에 담기 (`printDensity: "compact"`)

행이 많은 대장은 기본 치수(`.lg-cell` 22pt)로는 여러 장이 됩니다. 사양에
`printDensity: "compact"` 를 주면 인쇄 루트에 `lg-dense` 클래스가 붙어 조밀한 치수
(본문 6.5pt / 행 10pt)를 씁니다. **그 서식에만 적용되고 다른 대장의 정본 밀도는
그대로입니다.**

- A4 세로 여백 10/12mm → 쓸 수 있는 높이 275mm. 헤더·정보행·표머리·이상내역·범례·
  꼬리가 약 180pt 를 쓰므로 본문에 남는 것이 600pt 입니다. 행 수로 나눠 행 높이를 정합니다.
- `rowPageBreak` 가 있으면 섹션이 나뉘어 1쪽에 담을 수 없으니 함께 지웁니다.
...
2026-09-09 기준 대장 10종의 정본 장수: `DKJ-S-02-05` 6장(열을 3쪽으로 나눔),
`DKJ-S-02-31` 3장, `DKJ-S-02-09` 2장, 나머지 7종 1장.
