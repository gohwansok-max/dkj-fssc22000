...
### O/X 서식의 등록형 select (`__register__`)

`js/dkj-ox-form.js` 는 사양의 `fields[].options` 안에 `{"value": "__register__", "label": "+ 새 OO 등록"}`
가 있는 select 필드를 만나면, 그 필드를 **현장에서 계속 늘어나는 값**으로 보고 등록·관리
기능을 자동으로 붙입니다. 사양에 옵션을 하나하나 나열해 둘 수 없는 값(운행 차량처럼
새로 생기는 개체)에 씁니다 — 반대로 근무조·점검구역·보관장소처럼 **회사가 정해 둔
고정 카테고리**는 그냥 보통 select 로 두면 됩니다(값을 다 알고 있고 안 늘어나므로).

- 등록된 값은 `localStorage`(`dkj:registry:<서식코드>:<필드id>:v1`)에 저장됩니다.
  직원 계정 디렉터리처럼 시스템 관리자가 관리하는 정본이 아니라 현장에서 즉석으로
  느는 목록이라 클라우드 동기화 대상이 아닙니다 — 기기마다 따로 쌓입니다.
- `+ 새 OO 등록` 을 고르면 `prompt()` 로 값을 받아 레지스트리에 더하고 곧바로 선택
  상태로 만듭니다. select 옆에 자동으로 붙는 **"목록 관리(수정·삭제)"** 링크를 누르면
  등록된 값을 줄바꿈으로 구분한 텍스트로 보여주고, 사용자가 고친 대로 반영합니다 —
  줄을 지우면 삭제, 글자를 고치면 수정, 줄을 더하면 등록. 오타로 등록한 값을 빼거나
  고치는 길이 이것 하나뿐입니다(2026-09-10, 오타로 등록한 차량번호를 못 지운다는
  보고로 추가).
- **이미 저장된 기록은 손대지 않습니다.** 레지스트리를 정리해도 바뀌는 건 다음에
  고를 수 있는 목록뿐입니다 — 과거 기록의 값을 프로그램이 고치면 기록 변조이기
  때문입니다. 이미 틀리게 저장된 기록은 그 서식을 다시 열어 손으로 정정해야
  합니다(아직 잠기지 않은 임시저장 상태면 값만 바꿔 다시 저장하면 됩니다).
- 저장된 기록을 불러왔는데 그 값이 레지스트리에 없으면(다른 기기에서 등록했거나
  이 기능이 생기기 전 기록) 그 값도 자동으로 레지스트리에 편입합니다 — 불러온
  기록이 빈 선택으로 보이는 일을 막습니다.

지금은 `DKJ-S-02-18`(운송차량 위생점검표)의 차량번호 하나만 이 패턴을 씁니다. 같은
엔진을 쓰는 나머지 4종(`DKJ-S-02-06`·`DKJ-S-02-12`·`DKJ-S-02-29`·`DKJ-STORE-01`)의
select 필드는 근무조·점검구역·단위·보관장소처럼 전부 고정 카테고리라 해당 없습니다.
...
| `DKJ-QC-001` | `DKJ-H-01-01` (CCP-1BC 소독헹굼 일지) | 잔류염소(c09)·침지시간(c10)·헹굼 적합여부(c11) |
| `DKJ-QC-001` | `DKJ-H-01-02` (CCP-2P 금속검출 일지) | Fe/SUS 시편 규격(c18/c19)·복합검출(c20)·제품단독통과(c21) |
...
`DKJ-QC-001` 의 CCP-1BC·CCP-2P 연동(2026-09-10) — 온도일보 연동과 같은 원칙(불러올 때
재적용 안 함·값 없으면 비움·이탈 검증 통과·배지 화면 전용)을 그대로 따르되, 매칭 방식이
다릅니다. 온도일보는 회차가 고정(1~3차)이라 인덱스로 맞췄지만, CCP-1BC(`DKJ-H-01-01`)와
CCP-2P(`DKJ-H-01-02`)는 행을 자유롭게 추가·삭제하고 매 회차 점검시각을 직접 입력하는
구조라 행 수·순서가 날마다 다릅니다. 그래서 **QC-001의 회차별 목표시각과 가장 가까운
시각의 행**을 찾아 매칭합니다(`nearestRowByTime()`) — 인덱스(1번째 행 = 1차) 매칭이 아닙니다.

- **c12(소독액·헹굼수 교체)는 연동하지 않습니다.** CCP-1BC 쪽에 회차별 값이 없고
  하루 단위 `waterChangeTimes` 문자열(교체 시각 나열)만 있어, QC-001의 회차별 항목에
  자연스럽게 대응시킬 칸이 없습니다. 온도일보 연동의 "3차 연동 없음"과 같은 이유로
  억지로 끼워 맞추지 않고 수기 입력으로 남겼습니다.
- **c11(헹굼 적합여부)은 CCP-1BC 원본에 없는 파생값**입니다 — CCP-1BC 자신의 판정
  기준(`rinseSecOk`: 50~60초, `residualClOk`: 잔류염소 4ppm 미만)을 QC-001 쪽에서
  그대로 복제해(`ccp1RinseOk()`) 그 행의 침지시간·잔류염소가 둘 다 기준 안이면 '○',
  하나라도 벗어나면 '×'로 계산해 채웁니다.
- **c18/c19(Fe/SUS 시편 규격)는 회차가 아니라 그날 하루 값**입니다 — CCP-2P 기록의
  `feSize`/`susSize`(레코드 최상위, 회차별 아님)를 그대로 가져오므로 세 회차 모두 같은
  값이 채워집니다.
- **c20(복합검출)은 CCP-2P의 두 항목(제품+Fe, 제품+SUS)을 합친 값**입니다 — 그 서식은
  이 둘을 따로 O/X 로 기록하는데 QC-001은 "복합 검출" 한 항목뿐이라, 하나라도 '×'면
  '×', 둘 다 '○'면 '○'로 합칩니다. CCP-2P의 "시편 단독"(Fe만/SUS만) 항목은 QC-001에
  대응 칸이 없어 사용하지 않습니다.
