...
def convert_record(form_id: str, record: dict[str, Any], uid_map: dict[str, str], problems: list[str]) -> tuple[str, dict[str, Any]]:
    ...
    if not owner_uid:
        owner_uid = uid_for(data.get("createdByEmpId"), uid_map, f"{form_id}/{record_id} 최초작성자", problems)
    data["id"] = record_id
    data["formId"] = form_id
    ...
    for encoded_form, wrapper in legacy.items():
        ...
        for raw_record in wrapper["value"]:
            if not isinstance(raw_record, dict):
                ...
                continue
            record_key, payload = convert_record(form_id, raw_record, uid_map, problems)
