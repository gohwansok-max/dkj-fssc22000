...
`localStorage` 에 저장합니다. 빌드·번들러·테스트 단계가 없습니다.

> **클라우드 동기화는 아직 켜지 않았습니다.** `js/dkj-firebase-config.js` 가 비어 있어
> 지금은 기록이 **그 태블릿에만** 저장되고 직원별 로그인도 뜨지 않습니다. 값을 채우면
> Firebase RTDB 기기 간 동기화와 로그인·결재 서명 검증이 함께 켜집니다. 절차는 그 파일
> 맨 위 주석에 있습니다. 그때까지는 기록보관함의 **전체 백업(JSON)** 을 주 1회 받으세요.
